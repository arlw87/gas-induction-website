...
export const randomNumberArray = (numOfElements, lengthOfArray) => {
  const starterArray = [];
  const max = featured.length;

  let index = 0;

  while (starterArray.length < 3) {
    let ran = generateRandomNumber(lengthOfArray);
    if (!starterArray.includes(ran)) {
      starterArray[index] = ran;
      index++;
    }
  }
